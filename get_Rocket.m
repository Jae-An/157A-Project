...
geo = rocket.geo;
    ...
    % (nose bulkhead, drogue, payload bulkhead, avionics, main, bulkhead,
    %  plumbing, nozzle)

    geo = rocket.geo;
    
    geo.misc.L(1:7) = [0.5/12, 3.5/12, 0.5/12, 5.375/12, 8.5/12, 0.5/12, 0.5]; % ALL ESTIMATES
    ...
    geo.misc.x(1) = geo.nose.L * (3.252/12 / geo.nose.D); %nose payload D of 3.252"
    geo.misc.x(2) = geo.misc.x(1) + geo.misc.L(1);
    geo.misc.x(3) = geo.nose.L + geo.nose.D - geo.misc.L(4) - geo.payload.L - geo.misc.L(3);
    geo.misc.x(4) =  geo.nose.L + geo.nose.D - geo.misc.L(4);
    geo.misc.x(5) = geo.misc.x(4) + geo.misc.L(4);
    geo.misc.x(6) = geo.misc.x(5) + geo.misc.L(5);
    geo.misc.x(7) = geo.ox_t.x + geo.ox_t.L;
    
    % Payload
    geo.payload.x = geo.misc.x(3) + geo.misc.L(3);
...
W = rocket.weight;       
    ...
    W.misc.W(1:7) = [1.7, 0.44, 0.42, 2.3, 0.92, 1.7, 3];
